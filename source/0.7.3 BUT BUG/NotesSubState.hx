...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		tip.setFormat(Paths.languageFont(), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip.borderSize = 2;
		add(tip);

		tipTxt = new FlxText(tipX, tipY + 24, 0, '', 16);
		tipTxt.setFormat(Paths.languageFont(), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipTxt.borderSize = 2;
		add(tipTxt);
		updateTip();
	}
	...
	override function update(elapsed:Float) {
		...
		if(holdingOnObj != null)
		{
			...
			else if (generalMoved || generalPressed)
			{
				...
				{
						var centerX = colorWheel.x + colorWheel.width / 2;
						var centerY = colorWheel.y + colorWheel.height / 2;
						var mouseX = pointerX();
						var mouseY = pointerY();
						...
						var maxDist = colorWheel.width / 2;
						var sat = FlxMath.bound(dist / maxDist, 0, 1);
					if(sat != 0) 
						setShaderColor(FlxColor.fromHSB(degrees, sat, _storedColor.brightness));
					else 
		...
		else if(controls.RESET && hexTypeNum < 0)
		{
			if(FlxG.keys.pressed.SHIFT)
			{
				for (i in 0...3)
				{
					...
					var color:FlxColor = !onPixel ? ClientPrefs.data.defaultData.arrowRGB[curSelectedNote][i] :
													ClientPrefs.data.defaultData.arrowRGBPixel[curSelectedNote][i];
					switch(i)
					{
						case 0:
			...
			setShaderColor(!onPixel ? ClientPrefs.data.defaultData.arrowRGB[curSelectedNote][curSelectedMode] : ClientPrefs.data.defaultData.arrowRGBPixel[curSelectedNote][curSelectedMode]);
			FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
			updateColors();
		}
	}
	...
	function updateColors(specific:Null<FlxColor> = null)
	{
		...
		colorWheelSelector.setPosition(
			colorWheel.x + colorWheel.width/2 - colorWheelSelector.width/2, 
			colorWheel.y + colorWheel.height/2 - colorWheelSelector.height/2
		);
		if(wheelColor.brightness != 0)
		{
			var hueWrap:Float = wheelColor.hue * Math.PI / 180;
			colorWheelSelector.x += Math.cos(hueWrap) * (colorWheel.width/2) * wheelColor.saturation;
			colorWheelSelector.y += -Math.sin(hueWrap) * (colorWheel.height/2) * wheelColor.saturation;
		}
