...
private static final cmy:Int = 20;
	...
	override public function create()
	{
		...
		scoreTxt.setFormat(Paths.enfont("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		}else
		{
...
msTxtKade = new FlxText(520, 350, 0, "", 19);
msTxtKade.alpha = 0;
...
var psychEngineVersion:String = MainMenuState.psychEngineVersion;
	...
	public function updateScore(miss:Bool = false)
	{
		...
		+ ' | Accuracy:'+ (ratingName != '?' ? ' ${Highscore.floorDecimal(ratingPercent * 100, 2)}% | $ratingFC ' : '') + '($ratingName)';
		}
		else{
		scoreTxt.text = '分数: ' + songScore
		+ ' | 连击中断: ' + songMisses
		+ ' | 准确度:'+ (ratingName != '?' ? ' ${Highscore.floorDecimal(ratingPercent * 100, 2)}% | $ratingFC ' : '') + '($ratingName)';
		}
