...
class PsychUIRadioGroup extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		...
		if(maxItems > 0 && maxItems < labels.length && FlxG.mouse.wheel != 0 && PsychUIEventHandler.overlaps(_hitbox, camera))
		{
		...
		{
			if(hasArrowUp && maxItems > 0 && curScroll > 0 && PsychUIEventHandler.overlaps(arrowUp, camera))
			{
				curScroll--;
				arrowUp.animation.play('press');
			}
			else if(hasArrowDown && maxItems > 0 && curScroll < (labels.length - maxItems) && PsychUIEventHandler.overlaps(arrowDown, camera))
			{
				curScroll++;
