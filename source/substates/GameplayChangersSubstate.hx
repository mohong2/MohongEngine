...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	function getOptions()
	{
		var goption:GameplayOption = new GameplayOption(
			Language.get('Scroll Type', 'Scroll Type'),
			'scrolltype',
			'string',
			'multiplicative',
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Scroll Speed', 'Scroll Speed'),
			'scrollspeed',
			'float',
			1
		);
		...
		#if !html5
		var option:GameplayOption = new GameplayOption(
			Language.get('Playback Rate', 'Playback Rate'),
			'songspeed',
			'float',
			1
		);
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Health Gain Multiplier', 'Health Gain Multiplier'),
			'healthgain',
			'float',
			1
		);
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Health Loss Multiplier', 'Health Loss Multiplier'),
			'healthloss',
			'float',
			1
		);
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Instakill on Miss', 'Instakill on Miss'),
			'instakill',
			'bool',
			false
		);
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Practice Mode', 'Practice Mode'),
			'practice',
			'bool',
			false
		);
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Play as Opponent', 'Play as Opponent'),
			'playOpponent',
			'bool',
			false
		);
		...
		var option:GameplayOption = new GameplayOption(
			Language.get('Reverse the goodNoteHit and opponentNoteHit script functions', 'Reverse the goodNoteHit and opponentNoteHit\n script functions'),
			'reverseNoteHit',
			'bool',
			false
		);
	...
	public function new()
	{
		...
		{
			var optionText:FlxTextMenuItem = new FlxTextMenuItem(200, 360, optionsArray[i].name, fontSize);
			...
			if(optionsArray[i].type == 'bool') {
				...
				optionText.snapToPosition();
				var valueText:FlxTextAttached = new FlxTextAttached(Std.string(optionsArray[i].getValue()), fontSize, optionText.width + 35, 0);
					...
					if(holdTime > 0.5 || pressed) {
						if(pressed) {
							...
							switch(curOption.type)
							{
								...
								{
									var oOption:GameplayOption = getOptionByVariable("scrollspeed");
									if (oOption != null)
									{
	...
	function updateTextFrom(option:GameplayOption) {
		...
		if (option.child != null) {
			...
			if (parentText != null) {
				option.child.offsetX = parentText.width + 35;
			}
		}
	}
...
class GameplayOption
{
	...
	public var variable:String = null; //Variable from ClientPrefs.data.hx's gameplaySettings
