...
class OldPauseSubState extends MusicBeatSubstate
{
		...
		if (accepted && (cantUnpause <= 0 || !ClientPrefs.data.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case "Exit to menu":
					...
					Paths.currentModDirectory = states.MainMenuState.selectedModFolder;
	...
	public static function restartSong(noTrans:Bool = false)
	{
		...
		{
			// Restart song via loading state to allow async texture pre-decoding
			states.LoadingState.loadAndSwitchState(new PlayState(), true);
		}
	}
