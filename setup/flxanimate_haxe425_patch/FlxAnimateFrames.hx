...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	public static function sliceFrame(name:String, rotated:Bool, dimensions:FlxRect, ?sourceSize:FlxPoint, ?offset:FlxPoint, Frames:FlxAtlasFrames)
	{
		...
		{
			var srcSize:FlxPoint = (sourceSize != null) ? sourceSize : FlxPoint.get(dimensions.x, dimensions.y);
			var off:FlxPoint;
			if (offset != null) off = offset;
			else off = FlxPoint.get();
			off.set(-off.x, -off.y);
			Frames.addAtlasFrame(dimensions, srcSize, off, name, (rotated) ? ANGLE_NEG_90 : ANGLE_0);
		}
	}
