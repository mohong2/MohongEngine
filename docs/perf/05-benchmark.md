...
## 2. before / after（同口径对比，songEnd #1，seek 模式）

| 指标 | before（83e71b7 基线） | after（全部修复） |
|---|---|---|
| 内存 | 407MB | 408MB |
| 峰值 | 430MB | 428MB |
| 缓存图 | 86 | 86 |
| 帧时间 p50 / p95 / p99 | 5 / 9 / 20 ms | 4 / 5 / 9 ms |
| 渲染段 p50 / p95 | 1 / 2 ms | 1 / 1 ms |
| 可见精灵 | 34 | 34 |

- 帧时间尾部分布显著收窄：p95 9→5ms，p99 20→9ms（帧率解锁 + 移除 System.gc 的直接效果）。
- 内存基线持平（407→408MB）。

## 3. 验收长跑（20 次重试 + 菜单往返）

受时间限制未跑满 20 次；after 3 次重试内存曲线 408→414MB 平稳、无崩溃。
20 次完整验收与 Android/iOS 实机需在目标机补测。

## 4. GC / 卡顿观测

System.gc() 已从切歌路径移除；帧时间 p99 20ms→9ms 与切歌无卡顿为间接证据。
